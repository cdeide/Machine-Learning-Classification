...
def train_test_split(X, y, test_size=0.33, random_state=None, shuffle=True):
    ...
    # Shuffle if True
    if shuffle:
        X_copy, y_copy = myutils.shuffle_data(random_state, X_copy, y_copy)
...
def kfold_split(X, n_splits=5, random_state=None, shuffle=False):
    ...
    # Shuffle if True
    if shuffle:
        X_copy = myutils.shuffle_data(random_state, X_copy)

    # Split the data
    splits = myutils.split_data(n_splits, X_copy)

    # "Fold" splits into tuples
    folds = myutils.fold_data(n_splits, splits)
...
def stratified_kfold_split(X, y, n_splits=5, random_state=None, shuffle=False):
    ...
    # Shuffle if True
    if shuffle:
        X_copy = myutils.shuffle_data(random_state, X_copy, y_copy)

    # Perform group by
    grouped_indices = myutils.group_by(y_copy)

    # Now deal out indexes
    splits = myutils.split_stratified_data(n_splits, grouped_indices)
